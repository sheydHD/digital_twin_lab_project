# Authors and Contributors

This project is developed and maintained by students at RWTH Aachen University as part of the Digital Twins Lab.

## Core Developers

### Antoni Dudij
- **Email**: antonidudij@rwth-aachen.de
- **Role**: Lead Developer & Maintainer
- **Affiliation**: RWTH Aachen University, M.Sc. Computational Engineering Science (CES)
- **Contributions**: Project architecture, FEM solver,  analysis pipeline, documentation

### Maksim Feldmann
- **Email**: maksim.feldman@rwth-aachen.de
- **Role**: Core Developer
- **Affiliation**: RWTH Aachen University, M.Sc. Computational Engineering Science (CES)
- **Contributions**: Model development, Bayesian inference implementation

---

## How to Contribute

We welcome contributions from the community! Please see our [Contributing Guidelines](CONTRIBUTING.md) for details on how to:

- Report bugs
- Suggest new features
- Submit pull requests
- Improve documentation

## Acknowledgments

This project was developed as part of the **Digital Twins Lab** coursework at RWTH Aachen University.

### Special Thanks

- **PyMC Development Team** - For the probabilistic programming framework
- **ArviZ Team** - For MCMC diagnostics and visualization tools
- **Digital Twins Lab Faculty** - For project supervision and guidance

---

## Contact

- **Issues & Bug Reports**: [GitHub Issues](https://github.com/sheydHD/digital_twin_lab_project/issues)
- **Discussions**: [GitHub Discussions](https://github.com/sheydHD/digital_twin_lab_project/discussions)
- **Security Issues**: See [SECURITY.md](SECURITY.md)

---

## Contributor List

The following people have contributed to this project:

- **Antoni Dudij** (@sheydHD) - Lead Developer
- **Maksim Feldmann** - Core Developer

*Want to see your name here? Contribute to the project!*

---

<div align="center">

**Digital Twins Lab - RWTH Aachen University**

*Bridging physics-based modeling and data-driven inference for structural health monitoring*

</div>
