# Digital Twin Bayesian Model Selection - Documentation

<div align="center">

[![Documentation](https://img.shields.io/badge/docs-latest-blue.svg)](.)
[![Python 3.10+](https://img.shields.io/badge/python-3.10+-blue.svg)](https://www.python.org/downloads/)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](../LICENSE)

**Comprehensive documentation for the Bayesian beam theory model selection framework**

[Getting Started](#getting-started) • [Architecture](#architecture) • [API Reference](#api-reference) • [Contributing](#contributing)

</div>

---

## 📖 Documentation Overview

This directory contains all technical documentation for the project, organized for easy navigation.

### 🚀 Quick Links

| Document | Description | Audience |
|----------|-------------|----------|
| **[Getting Started](GETTING_STARTED.md)** | Installation and first run | Everyone |
| **[Architecture](ARCHITECTURE.md)** | System design and components | Developers |
| **[API Reference](API.md)** | Complete API documentation | Developers |
| **[Development Guide](DEVELOPMENT.md)** | Development workflow | Contributors |
| **[Contributing](CONTRIBUTING.md)** | How to contribute | Contributors |
| **[Changelog](CHANGELOG.md)** | Version history | Everyone |
| **[Security Policy](SECURITY.md)** | Security guidelines | Maintainers |

---

## 🎯 Find What You Need

### I Want To...

<details>
<summary><b>Understand the Project</b></summary>

- 📘 **[Project Overview](../README.md#overview)** - High-level introduction
- 📝 **[Explanation](EXPLANATION.md)** - Plain-English guide
- 🎓 **[Key Concepts](../README.md#key-concepts)** - Beam theory basics
- 📊 **[Results](../README.md#key-results)** - What we discovered

</details>

<details>
<summary><b>Get Started</b></summary>

- ⚡ **[Quick Start](GETTING_STARTED.md#quick-start)** - 5-minute setup
- 💻 **[Installation](GETTING_STARTED.md#installation)** - Detailed setup
- 🔧 **[Configuration](GETTING_STARTED.md#configuration)** - Customize settings
- ▶️ **[Running](GETTING_STARTED.md#running)** - Execute pipeline

</details>

<details>
<summary><b>Understand the Architecture</b></summary>

- 🏗️ **[Architecture Overview](ARCHITECTURE.md#high-level-architecture)** - System design
- 🔄 **[Data Flow](ARCHITECTURE.md#data-flow-diagram)** - How data moves
- 🧩 **[Components](ARCHITECTURE.md#core-components)** - Module breakdown
- ⚖️ **[Design Decisions](ARCHITECTURE.md#design-decisions--trade-offs)** - Why we built it this way

</details>

<details>
<summary><b>Use the API</b></summary>

- 📚 **[API Reference](API.md)** - Complete function reference
- 💡 **[Usage Examples](API.md)** - Code examples
- 🔢 **[Type Signatures](API.md)** - Type hints and validation
- ⚠️ **[Error Handling](API.md#error-handling)** - Exception guide

</details>

<details>
<summary><b>Develop & Contribute</b></summary>

- 🛠️ **[Development Setup](DEVELOPMENT.md)** - Dev environment
- 🧪 **[Testing Guide](DEVELOPMENT.md#testing)** - How to test
- 🐛 **[Debugging](DEVELOPMENT.md#debugging-guide)** - Troubleshooting
- 🤝 **[Contributing](CONTRIBUTING.md)** - Contribution workflow
- 📋 **[Code Standards](CONTRIBUTING.md#coding-standards)** - Style guide

</details>

<details>
<summary><b>Stay Updated</b></summary>

- 📰 **[Changelog](CHANGELOG.md)** - What's new
- 🔐 **[Security](SECURITY.md)** - Security policy
- 🗺️ **[Roadmap](#roadmap)** - Future plans

</details>

---

## 📚 Reading Paths

### For **New Users**

```
1. README.md (root) → Project overview
2. EXPLANATION.md → Plain-English guide  
3. GETTING_STARTED.md → Installation & first run
4. ../README.md#expected-results → What to expect
```

### For **Researchers**

```
1. README.md → Research context
2. ARCHITECTURE.md → Technical methodology
3. ../README.md#key-results → Findings
4. CHANGELOG.md → Implementation details
```

### For **Developers**

```
1. ARCHITECTURE.md → System design
2. DEVELOPMENT.md → Dev setup
3. API.md → Code reference
4. CONTRIBUTING.md → Workflow
```

### For **Contributors**

```
1. CONTRIBUTING.md → Start here
2. DEVELOPMENT.md → Development guide
3. ARCHITECTURE.md#extensibility-points → How to extend
4. API.md → API to implement
```

---

## 🏗️ Documentation Structure

```
docs/
├── README.md                    # This file - Documentation hub
├── GETTING_STARTED.md           # Installation and quick start
├── EXPLANATION.md               # Plain-English explanation
├── ARCHITECTURE.md              # System architecture
├── API.md                       # API reference
├── DEVELOPMENT.md               # Development guide
├── CONTRIBUTING.md              # Contribution guidelines
├── CHANGELOG.md                 # Version history
├── SECURITY.md                  # Security policy
└── INDEX.md                     # Legacy navigation index
```

---

## 🎓 Learning Path

### Beginner → Intermediate → Advanced

**Level 1: Understanding** 
- Read [EXPLANATION.md](EXPLANATION.md)
- Read [Getting Started](GETTING_STARTED.md)
- Run the pipeline once

**Level 2: Using**
- Study [Configuration](GETTING_STARTED.md#configuration)
- Experiment with aspect ratios
- Analyze results

**Level 3: Developing**
- Read [ARCHITECTURE.md](ARCHITECTURE.md)
- Read [DEVELOPMENT.md](DEVELOPMENT.md)
- Write your first test

**Level 4: Contributing**
- Read [CONTRIBUTING.md](CONTRIBUTING.md)
- Pick an issue
- Submit your first PR

---

## 🔗 External Resources

### Bayesian Inference
- [PyMC Documentation](https://www.pymc.io/) - Official PyMC docs
- [Bayesian Data Analysis](http://www.stat.columbia.edu/~gelman/book/) - Gelman et al.
- [Statistical Rethinking](https://xcelab.net/rm/statistical-rethinking/) - McElreath

### Beam Theory
- [Timoshenko Beam Theory](https://en.wikipedia.org/wiki/Timoshenko%E2%80%93Ehrenfest_beam_theory)
- [Euler-Bernoulli Beam Theory](https://en.wikipedia.org/wiki/Euler%E2%80%93Bernoulli_beam_theory)

### Finite Element Method
- [Introduction to FEM](https://www.colorado.edu/engineering/CAS/courses.d/IFEM.d/)
- [FEM for Beginners](https://www.simscale.com/docs/simwiki/fea-finite-element-analysis/what-is-fea-finite-element-analysis/)

### Python Development
- [The Hitchhiker's Guide to Python](https://docs.python-guide.org/)
- [Real Python](https://realpython.com/)
- [Python Testing with pytest](https://pragprog.com/titles/bopytest/python-testing-with-pytest/)

---

## 🗺️ Roadmap

### v1.1.0 (Q2 2026)
- [ ] Frequency-domain analysis
- [ ] Additional boundary conditions
- [ ] Real experimental data validation
- [ ] GPU acceleration for MCMC

### v2.0.0 (Q4 2026)
- [ ] 3D beam elements
- [ ] Multiple loading scenarios
- [ ] Real-time digital twin integration
- [ ] Web-based visualization dashboard

See [GitHub Issues](https://github.com/sheydHD/digital_twin_lab_project/issues) for details.

---

## 🤝 Contributing to Documentation

Documentation improvements are always welcome!

### Quick Fixes
- Found a typo? → Fix it and open a PR
- Broken link? → Update it
- Unclear explanation? → Open an issue

### Major Changes
1. Open an issue first to discuss
2. Follow the [documentation style guide](CONTRIBUTING.md#documentation)
3. Update related documents
4. Test all links
5. Submit PR

---

## 📧 Support

- **Questions**: [GitHub Discussions](https://github.com/sheydHD/digital_twin_lab_project/discussions)
- **Bug Reports**: [GitHub Issues](https://github.com/sheydHD/digital_twin_lab_project/issues)
- **Security**: See [SECURITY.md](SECURITY.md)

---

## 📄 License

This documentation is part of the Digital Twin Bayesian Model Selection project, licensed under the [MIT License](../LICENSE).

---

<div align="center">

**[⬆ Back to Top](#digital-twin-bayesian-model-selection---documentation)**

Made with ❤️ by the Digital Twins Lab

</div>
